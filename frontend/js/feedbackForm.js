document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (isUpdate) {
    try {
      ...
      document.querySelector(
        `input[name="teachingStyle"][value="${feedback.ratings.teachingStyle}"]`
      ).checked = true;
      document.querySelector(
        `input[name="courseContent"][value="${feedback.ratings.courseContent}"]`
      ).checked = true;
      document.querySelector(
        `input[name="courseDifficulty"][value="${feedback.ratings.courseDifficulty}"]`
      ).checked = true;
      document.querySelector(
        `input[name="teacherSincerity"][value="${feedback.ratings.teacherSincerity}"]`
      ).checked = true;
      document.querySelector(
        `input[name="studentEngagement"][value="${feedback.ratings.studentEngagement}"]`
      ).checked = true;
  ...
  feedbackForm.addEventListener("submit", async (event) => {
    ...
    if (
      !ratings.teachingStyle ||
      !ratings.courseContent ||
      !ratings.courseDifficulty ||
      !ratings.teacherSincerity ||
      !ratings.studentEngagement
    ) {
    ...
    try {
      ...
      if (response.ok) {
        window.location.href = `/studentDashboard/${std_id}`; // Redirect to student dashboard after submission
      }
    } catch (error) {
